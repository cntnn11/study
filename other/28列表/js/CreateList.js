function CreateList() {
	this.oFirstPane = document.createElement("div");
...
CreateList.prototype = {
		...
		while(aData[0]) {
			...
			for(i = 0; i < project.length; i++) {
				if(project[i].href) {
					...
					oElem.innerHTML =  " <a href=\"" + project[i].href + "\" target=\"_self\">" + project[i].text + "</a>"
				}
				else {
			...
			this.oFirstPane.appendChild(oDl);
			aData.shift()
		}
		this.oFirstPane.id = "firstpane";
		/*this.copyright.id = "copyright";
		this.copyright.innerHTML = "By &mdash; Ferris";*/
		document.getElementById("kcList01").appendChild(this.oFirstPane);
		/*document.body.appendChild(this.copyright)*/
	},

	click: function() {
		var that = this;
		this.oFirstPane.onclick = function(event) {
	...
	buffer: function(obj, iTarget, callback) {
		obj.iSpeed = (iTarget - obj.offsetHeight) / 1;
